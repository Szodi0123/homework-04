...
#Beállítom, hogy csak a hónapokat mutassa, és aszerint sorba rendezem.
tweets$time <- as.Date(tweets$time)
tweets_sorted <- tweets[order(tweets[, 6]),]
tweets_sorted$months <- strftime(tweets_sorted$time, format = "%m")
